...
/**
 * Base class for classes implementing representations of optional parameters of
 * various types. Acronym TLV means Tag-Length-Value. Derived from ByteData this
 * class "knows" how to parse from ByteBuffer and how to create a ByteBuffer
 * from it's data. Descanedants of the class (concrete types) should rewrite
 * <code>setValueData</code> and <code>getValueData</code> methods for setting
 * and getting of the particular data type. The tag and the length is set and
 * get by the <code>TLV</code> class. The class also provides additiona methods
 * for checking various type validity.
 * 
 ...
 * 02-10-01 ticp@logica.com removed some historical unused code 01-11-01
 * ticp@logica.com TLV length now returned as int to allow express the lengths >
 * 32767 (they're stored as negative values in vars of type short)
 */

public abstract class TLV extends ByteData
{
	private static String	attachLengthForOptParam	= null;
	 ...
	 */
	private boolean			valueIsSet				= false;			// must
																		// be
																		// set
																		// by
																		// setValueData()
	 ...
	 *                if the buffer contains somehow invalid data
	 */
	 ...
	 *                if the TLV wasn't set any data then there is nothing to be
	 *                returned (the optional parameter isn't transmitted then.)
	 */
	 ...
	 * 
	 * @exception ValueNotSetException
	 *                as this method uses <code>getValueData</code> for
	 *                calculation of the length this exception can be thrown
	 * @see #getValueData()
	 */
