 ...
 */
public class AgeNetworkCampaignImpl extends CampaignImpl
{
	public int processMessage(Connection connection, CampaignEntry campaign, CampaignMessage request) throws Exception
	{
		...
		try
		{
			...
			{
				String unit = campaign.getParameters().getString("campaign.unit", "month");
				int period = campaign.getParameters().getInteger("campaign.period", 1);

				request.setQuantity(DateUtil.getDateDiff(request.getRegisterDate(), request.getOrderDate(), unit, period));

				if (request.getQuantity() >= 12)
				{
